...
def setupLDAPPlugin():
    ...
    manage_addLDAPMultiPlugin(
        ...
        bindpwd='****', 
        #_extra_user_filter='(!(objectclass=computer))',
        binduid_usage=1, 
...
def movePluginToHeadOfList(plugin_registry, plugin_type, plugin_id):
    ...
    index = [i for i in plugin_registry._getPlugins(interface)].index(plugin_id)
    while index > 0:
        plugin_registry.movePluginsUp(interface, [plugin_id])
        new_index = [i for i in plugin_registry._getPlugins(interface)].index(plugin_id)
        if new_index >= index: 
